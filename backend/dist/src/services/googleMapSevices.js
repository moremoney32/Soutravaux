...
async function initBrowser(headless = true) {
    ...
    const randomUA = (0, userAgents_1.getRandomUserAgent)();
...
// ============================================
async function navigateToGoogleMaps(page, query) {
    let villeValue = '';
    if (Array.isArray(query.ville)) {
        villeValue = query.ville[0] || '';
    }
    ...
    for (const searchQuery of searchTerms) {
        ...
        try {
            ...
            console.log('⚠️ Erreur:', error instanceof Error ? error.message : String(error));
...
// ============================================
async function waitForGoogleMapsResults(page) {
...
async function scrapeGoogleMapsWithOffset(query, offset, limit, existingBrowser, existingPage) {
    ...
    try {
        if (!browser || !page) {
            ...
            const context = await browser.newContext({
                userAgent: (0, userAgents_1.getRandomUserAgent)(),
                locale: 'fr-FR',
...
//# sourceMappingURL=googleMapSevices.js.map