...
exports.calculerMomentsNotifications = calculerMomentsNotifications;
exports.planifierNotificationsPourEvenement = planifierNotificationsPourEvenement;
exports.supprimerNotificationsPendantes = supprimerNotificationsPendantes;
function calculerMomentsNotifications(eventDate, startTime) {
    const schedules = [];
 ...
 */
async function planifierNotificationsPourEvenement(connection, // ← Recevoir la connexion en paramètre
eventId, eventDate, startTime, creatorSocieteId) {
    try {
        const schedules = calculerMomentsNotifications(eventDate, startTime);
        for (const schedule of schedules) {
            await connection.query(`INSERT INTO event_notifications 
 ...
 * ✅ MODIFIÉ : UTILISER LA CONNEXION EXISTANTE
 */
async function supprimerNotificationsPendantes(connection, eventId) {
    try {
        await connection.query(`DELETE FROM event_notifications 
       WHERE event_id = ? AND sent_at IS NULL`, [eventId]);
...
//# sourceMappingURL=notificationsSheduler.js.map