...
exports.getPreSocietes = getPreSocietes;
exports.getSocietes = getSocietes;
...
const SseServices_1 = require("./SseServices");
...
// }
// ===== RÉCUPÉRATION DES PRÉSOCIÉTÉS =====
async function getPreSocietes(role, activiteIds, departementIds) {
    ...
    `;
        ...
        if (departementIds && departementIds.length > 0) {
            // Créer une sous-requête pour récupérer les numéros de départements
            const [departments] = await conn.query(`SELECT numero FROM departements WHERE id IN (${departementIds.map(() => '?').join(',')})`, departementIds);
            const departmentNumbers = departments.map((d) => d.numero);
            if (departmentNumbers.length > 0) {
                // Filtrer les présociétés dont le CP commence par un des numéros de département
                const cpConditions = departmentNumbers.map(() => 'LEFT(p.cp, 2) = ?').join(' OR ');
                query += ` AND (${cpConditions})`;
...
// ===== RÉCUPÉRATION DES SOCIÉTÉS =====
async function getSocietes(role, activiteIds, departementIds) {
    const conn = await db_1.default.getConnection();
    try {
...
async function getActivites() {
    const conn = await db_1.default.getConnection();
    try {
        const [rows] = await conn.query(`SELECT id, nom FROM activites ORDER BY nom ASC`);
        return rows.map((row) => ({
            id: String(row.id),
            name: row.nom
        }));
...
async function getDepartements() {
    const conn = await db_1.default.getConnection();
    try {
        const [rows] = await conn.query(`SELECT id, numero, nom FROM departements ORDER BY numero ASC`);
        return rows.map((row) => ({
            id: String(row.id),
            name: row.nom,
            code: row.numero
...
async function sendNotification(payload) {
    ...
    try {
        ...
        const recipientSocieteIds = [];
        // ========================================
        // RÉCUPÉRATION DES IDs SOCIÉTÉS
        // ========================================
        // Présociétés
        if (payload.recipients.preSocieteIds && payload.recipients.preSocieteIds.length > 0) {
            const [preSocietes] = await conn.query(`SELECT p.id, p.name
         FROM presocietes p
         WHERE p.id IN (${payload.recipients.preSocieteIds.map(() => '?').join(',')})`, payload.recipients.preSocieteIds);
            // CHANGEMENT : On garde directement les IDs présociétés
            preSocietes.forEach((ps) => {
        ...
        if (payload.recipients.societeIds && payload.recipients.societeIds.length > 0) {
            const [societes] = await conn.query(`SELECT s.id, s.email
         FROM societes s
         WHERE s.id IN (${payload.recipients.societeIds.map(() => '?').join(',')})`, payload.recipients.societeIds);
        ...
        if (payload.notificationTypes.includes('push') && recipientSocieteIds.length > 0) {
            ...
            try {
                const oneSignalResponse = await (0, node_fetch_1.default)('https://onesignal.com/api/v1/notifications', {
                    method: 'POST',
                    headers: {
        ...
        if (payload.notificationTypes.includes('sse') && recipientSocieteIds.length > 0) {
            ...
            try {
                ...
                // CHANGEMENT : recipientSocieteIds
                sseSent = (0, SseServices_1.sendToMultipleUsers)(recipientSocieteIds, sseData);
...
// STATISTIQUES
async function getNotificationStats(role) {
    const conn = await db_1.default.getConnection();
    try {
        const [preSocietesCount] = await conn.query(`SELECT COUNT(*) as count FROM presocietes WHERE role = ?`, [role]);
        const [societesCount] = await conn.query(`SELECT COUNT(*) as count FROM societes WHERE role = ?`, [role]);
        return {
