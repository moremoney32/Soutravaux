...
exports.AnnonceurRegister = AnnonceurRegister;
exports.FournisseurRegister = FournisseurRegister;
...
async function UserRegister(data) {
    ...
    try {
        ...
        if (siret && siret.trim() !== "") {
            ...
            const [siretExists] = await conn.query(`SELECT 'societes' as source, id FROM societes WHERE siret = ? 
     UNION ALL 
     SELECT 'presocietes' as source, id FROM presocietes WHERE siret = ?`, [siret, siret]);
        ...
        const tempPassword = await bcrypt_1.default.hash("__PENDING__", 10);
        //Insertion du membre
        const [resMembre] = await conn.query(`INSERT INTO membres (
    ...
    verificationCode, verificationExpiry, ref
        ...
        const membreId = resMembre.insertId;
        if (siret || name) {
            const [resSociete] = await conn.query(`INSERT INTO presocietes (
     ...
     address, phonenumber, membre_id
        ...
        try {
            const response = await axios_1.default.post("https://integrations-api.solutravo-compta.fr/api/send_sms", {
...
async function VerifyCode({ email, code }) {
    const [rows] = await db_1.default.query("SELECT * FROM membres WHERE email = ?", [email]);
    const user = rows[0];
    if (!user)
    ...
    await db_1.default.query("UPDATE membres SET isVerified = true, verificationCode = NULL, verificationExpiry = NULL WHERE email = ?", [email]);
    return { email };
}
...
// }
async function CompleteRegistration({ email, passe }) {
    if (!validator_1.default.isStrongPassword(passe, { minLength: 8, minUppercase: 1, minNumbers: 1 })) {
        const err = new Error("Mot de passe trop faible");
        err.statusCode = 422;
        throw err;
    }
    ...
    const hashed = await bcrypt_1.default.hash(passe, 10);
    await db_1.default.query("UPDATE membres SET passe = ?, date_modification = NOW() WHERE email = ?", [hashed, email]);
    const [updatedRows] = await db_1.default.query("SELECT id, email, prenom FROM membres WHERE email = ?", [email]);
    const updatedUser = updatedRows[0];
    // ENVOI DE L'EMAIL DE BIENVENUE APRÈS INSCRIPTION COMPLÈTE
    try {
        const response = await axios_1.default.post("https://auth.solutravo-app.fr/send-email.php", {
...
async function sendOTPSMS(phoneNumber, otp, isResend = false) {
...
async function ResendVerificationCode(email) {
    ...
    try {
        await conn.beginTransaction();
        const [rows] = await conn.query(`SELECT id, email, prenom, nom, statut, verificationCode, verificationExpiry 
       FROM membres 
       WHERE email = ?`, [email]);
        const user = rows[0];
        if (!user) {
        ...
        const [societeRows] = await conn.query("SELECT phonenumber FROM presocietes WHERE membre_id = ?", [user.id]);
        ...
        const [updateResult] = await conn.query(`UPDATE membres 
       ...
       WHERE email = ?`, [otp, expiry, email]);
        ...
        try {
            ...
            const err = new Error(`Code généré mais SMS non envoyé: ${smsError.message}. Réessayez dans quelques instants.`);
            err.statusCode = 500;
            throw err;
        }
...
/*****userAnnonceur **/
async function AnnonceurRegister(data) {
    const { emailAnnonceur, firstNameAnnonceur, lastNameAnnonceur, role, phoneAnnonceur, headquartersAnnonceur, activityAnnonceur, companyNameAnnonceur, howDidYouKnowAnnonceur, siretAnnonceur, 
    // Colonnes supplémentaires existantes (optionnelles)
    size, legal_form, cp, ville, rue, capital } = data;
    console.log(data);
    ...
    if (!validator_1.default.isEmail(emailAnnonceur)) {
        const err = new Error("Email invalide");
        err.statusCode = 422;
        throw err;
    }
    ...
    try {
        ...
        if (siretAnnonceur && siretAnnonceur.trim() !== "") {
            ...
            const [siretExists] = await conn.query(`SELECT 'societes' as source, id FROM societes WHERE siret = ? 
     UNION ALL 
     SELECT 'presocietes' as source, id FROM presocietes WHERE siret = ?`, [siretAnnonceur, siretAnnonceur]);
            if (siretExists.length > 0) {
                const err = new Error("Ce SIRET est déjà associé à une société (en attente de validation ou active).");
                err.statusCode = 409;
                throw err;
        ...
        membre_id
            ...
            size ?? null,
            (legal_form ?? 'SARL'), // SARL par défaut pour annonceurs
            siretAnnonceur ?? null,
            (role ?? 'annonceur'),
            cp ?? null,
            ville ?? null,
...
async function FournisseurRegister(data) {
    const { contactEmail, contactFirstName, contactLastName, contactPosition, contactPhone, role, companyName, siret, address, postalCode, city, sector, customSector, message, cp, ville, rue } = data;
    console.log(data);
    // Validation des champs obligatoires
    if (!contactEmail) {
        const err = new Error("Email obligatoire");
    ...
    if (!validator_1.default.isEmail(contactEmail)) {
        const err = new Error("Email invalide");
        err.statusCode = 422;
        throw err;
    }
    ...
    try {
        ...
        try {
            ...
            const emailSolutravo = await axios_1.default.post("https://mail.api.elyft.tech/send-email.php", {
                receiver: "vincent@solutravo.fr",
