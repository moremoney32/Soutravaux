...
async function UserRegister(data) {
    ...
    try {
        ...
        const [exists] = await conn.query("SELECT id FROM membres WHERE email = ? AND role = ?", [email, role]);
        if (exists.length) {
            throw new Error("Ce rôle est déjà associé à cet email.");
        }
        // Gestion de la société
        let presocieteId = null;
        if (siret) {
            const [rows] = await conn.query("SELECT id FROM presociete WHERE siret = ?", [siret]);
            if (rows.length) {
                presocieteId = rows[0].id;
            }
            else {
                const [res] = await conn.query("INSERT INTO presociete (name, size, legal_form, siret) VALUES (?, ?, ?, ?)", [companyName || null, companySize || null, legalForm || null, siret]);
                presocieteId = res.insertId;
            }
        }
        else {
            if (!companyName) {
            ...
            const [res] = await conn.query("INSERT INTO presociete (name, size, legal_form) VALUES (?, ?, ?)", [companyName, companySize || null, legalForm || null]);
            presocieteId = res.insertId;
        }
        // Génération OTP
        const otp = genOTP();
      ...
      `
        ...
        // Insertion du membre
        await conn.query(`INSERT INTO membres (email, prenom, numero, lieu, role, presociete_id, verificationCode, verificationExpiry)
   VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [email, prenom, numero, lieu, role, presocieteId, otp, expiry]);
        await conn.commit();
        return { email };
    }
    catch (err) {
...
async function ResendVerificationCode(email) {
    ...
    try {
        ...
        await conn.query("UPDATE membres SET verificationCode = ?, verificationExpiry = ? WHERE email = ?", [otp, expiry, email]);
        await conn.commit();
        return { email };
    }
    catch (err) {
