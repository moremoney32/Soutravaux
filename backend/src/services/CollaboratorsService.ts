...
export async function getCollaboratorsBySociete(
  ...
  try {
    ...
    console.log("🟡 [CollaboratorsService] societeId =", societeId);
    ...
    console.log("🟢 Collaborateurs trouvés =", rows.length);
...
/**
 * Récupérer les collaborateurs uniques (sans doublons)
 */
export async function getUniqueCollaboratorsBySociete(
  ...
  try {
    ...
    console.log("🟡 [UniqueCollaborators] societeId =", societeId);
    ...
    console.log("🟢 Unique collaborateurs =", rows.length);

    return rows as Collaborator[];

  } catch (e) {
    console.error("❌ SQL ERROR CollaboratorsService:", e);
    throw e;
  } finally {
    conn.release();
  }
